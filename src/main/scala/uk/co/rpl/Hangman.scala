  ...
  def main(args: Array[String]): Unit = {
    ...
    while (true) {
      ...
      while (!word.isFailed && !word.isFound) {
        ...
        prline("enter one or nore letters followed by newline")
        ...
        for (char <- line.toUpperCase) {
          ...
          if (used)
            prline("That letter " + char + " has already been used")
      ...
      if (word.isFailed)
        prline("SORRY the word was  not found it was " + word.getWord)
      prline("\n\n-----------------------------\n\n try a new word\n")
    }
  }
